...
1. `materialize_reference_circuit(recipe_id="flyback_converter_cpp")` — confirm both files in workspace root.
2. `build_dll_device(source_path="flyback_controller.cpp")` — confirm `flyback_controller.dll` beside the schematic.
3. `inspect_schematic(schematic_path="Flyback_sim.qsch")` — X1 / `Flyback_controller`, `.tran 0 300µ 0 100n uic`.
4. Optional: `validate_dll_symbol_signature(schematic_path="Flyback_sim.qsch", reference="X1", source_path="flyback_controller.cpp")`.
5. `run_simulation(source_path="Flyback_sim.qsch")` — dry-run then run. When
   `QSPICE_EXE` is set, `generate_netlist` uses companion `QUX.exe -Netlist` so
   the DLL block (X1) is included automatically.
6. `list_signals` → `plot_waveforms` and/or `read_waveform` on `V(out)` (output rail).
