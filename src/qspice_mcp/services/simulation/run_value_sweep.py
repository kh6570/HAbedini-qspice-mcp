"""Service for sweeping one component value across multiple runs."""
...
SERVICE_SPEC = ServiceSpec(
    name="run_value_sweep",
    title="Run Value Sweep",
    summary="Run one schematic across multiple component values.",
    phase="implemented",
    read_only=False,
    long_running=True,
)
...
def run_value_sweep(
    ...
    *,
    workspace_root: Path,
    reference: str,
    values: tuple[str | int | float, ...] | list[str | int | float],
    ...
    """Run a synchronous sweep that changes one component value per run."""
    ...
    planned_runs = tuple(
        (
            ...
            f"{reference}={value}",
            {reference: value},
            _make_value_apply_edit(reference, value),
        )
...
__all__ = ["SERVICE_SPEC", "run_value_sweep"]
